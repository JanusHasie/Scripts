...
try :
    ...
    blockx_end = time.time()
    print("Variables created. Elapsed block 1: ",blockx_end - start_time)
...
blocky_end = time.time()
print("LOAD AND PIVOT COMPLETE. Elapsed from start: ", blocky_end - start_time,"""
      \n    From prev block: """, blocky_end - blockx_end)
...
one_months_ago = dfHS.columns[11]
two_months_ago = dfHS.columns[10]
three_months_ago = dfHS.columns[9]
four_months_ago = dfHS.columns[8]
five_months_ago = dfHS.columns[7]
six_months_ago = dfHS.columns[6]
seven_months_ago = dfHS.columns[5]
eight_months_ago = dfHS.columns[4]
nine_months_ago = dfHS.columns[3]
ten_months_ago = dfHS.columns[2]
eleven_months_ago = dfHS.columns[1]
twelve_months_ago = dfHS.columns[0]

# Assign forecast variables
one_months_fc = dfFC.columns[0]
two_months_fc = dfFC.columns[1]
three_months_fc = dfFC.columns[2]
four_months_fc = dfFC.columns[3]
five_months_fc = dfFC.columns[4]
six_months_fc = dfFC.columns[5]
seven_months_fc = dfFC.columns[6]
eight_months_fc = dfFC.columns[7]
nine_months_fc = dfFC.columns[8]
ten_months_fc = dfFC.columns[10]
eleven_months_fc = dfFC.columns[11]
twelve_months_fc = dfFC.columns[12]
blockx_end = time.time()
print("Variables created. Elapsed from start: ", blockx_end - start_time,"""
      \n    From prev block: """, blockx_end - blocky_end)
...
conn.commit()
blocky_end = time.time()
print("Forecast table dropped and recreated. Elapsed from start: ", blocky_end - start_time,"""
      \n    From prev block: """, blocky_end - blockx_end)
